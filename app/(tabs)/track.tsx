...
import React, { useEffect, useState } from 'react'; import { View, Text, StyleSheet, Image, FlatList, TouchableOpacity, Dimensions, TextInput, SafeAreaView, } from 'react-native'; import { Button } from 'react-native-paper'; import { useRouter } from 'expo-router'; import { Package } from '../../lib/types'; // your package type
...
const filtered = packages?.filter(pkg => pkg.id.toString().includes(search) || pkg.name.toLowerCase().includes(search.toLowerCase()) );
...
<View style={styles.statusRow}>
    {['awaiting', 'processing', 'out', 'delivered'].map(stage => (
      <View key={stage} style={styles.statusItem}>
        ...
        />
        <Text
          style={[styles.statusLabel, item.status === stage && styles.statusLabelActive]}
        >
          {stage === 'awaiting'
            ? 'Awaiting Drop-off'
            : stage === 'processing'
...
statusRow: { flexDirection: 'column', marginTop: 16 }, statusItem: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 }, dot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#444', marginRight: 8 }, dotActive: { backgroundColor: PURPLE }, statusLabel: { color: '#555' }, statusLabelActive: { color: '#fff', fontWeight: '500' },

actionRow: { flexDirection: 'row', marginTop: 16, justifyContent: 'space-between' }, actionBtn: { flex: 1, borderRadius: 8, paddingVertical: 12, alignItems: 'center' }, trackBtn: { backgroundColor: PURPLE, marginRight: 8 }, trackLabel: { color: '#fff', fontSize: 16 }, viewAllBtn: { backgroundColor: '#333' }, viewAllLabel: { color: '#fff', fontSize: 16 }, });
